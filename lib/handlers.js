...
handlers._users.POST = function (data, callback) {
    ...
    db.queries.getUser(phone, function (err, data) {
        ...
        // Store the user
        db.queries.postUser(userObj, function (err, isUserAdded) {
            if (!err && isUserAdded)
                callback(200);
            else if (!err && !isUserAdded)
                callback(500, { "Error": "unable to add user to the database" });
            else
                callback(500, { 'Error': "unknown error" });
        });
    })
}
...
handlers._users.GET = function (data, callback) {
    ...
    handlers._tokens.verifyToken(token, phone, function (tokenIsValid) {
        ...
        db.queries.getUser(phone, function (err, data) {
            if (err && !data)
                callback(520, { "Error": "unknown error" });
            else if (!err && data)
                callback(200, data);
            else
                callback(404);
...
handlers._tokens.POST = function (data, callback) {
    ...
    db.queries.getUser(phone, function (err, data) {
        if (!err && data) {
            ...
            let expires = Date.now() + 1000 * 60 * 60 * 24; // the token last for 1 day
                ...
                db.queries.deleteCheck(id, function (err) {
                    ...
                    db.queries.getUser(checkData.userphone, function (err, userData) {
                        if (err && !data)
                            ...
                            return callback(404, { "Error": "Could not find the user who created the check, so could not remove the check from the list of checks on their user object." });
